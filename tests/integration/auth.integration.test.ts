...
describe('Authentication Integration Tests', () => {
  ...
  describe('OAuth2 PKCE Flow Initiation', () => {
    test('should generate authentication URL with valid PKCE parameters', async () => {
      ...
      expect(result).toHaveValidAuthResponse();
    ...
    test('should handle authentication initiation failure gracefully', async () => {
      ...
      const result = await graphAdapter.startAuthentication('invalid-user');
      
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
      expect(result.error).toContain('MSAL configuration error');
    });
  ...
  describe('OAuth2 Callback Handling', () => {
    ...
    test('should handle token exchange failure', async () => {
      ...
      const mockMsalApp = createMockMsalApp();
      mockMsalApp.acquireTokenByCode.mockRejectedValue(
  ...
  describe('Token Management', () => {
    ...
    test('should clear tokens on logout', async () => {
      // Verify tokens exist
      const tokensBeforeLogout = await securityManager.getTokens('microsoft');
      ...
      const tokensAfterLogout = await securityManager.getTokens('microsoft');
  ...
  describe('Error Scenarios and Edge Cases', () => {
    test('should handle network failures during authentication', async () => {
      ...
      const mockMsalApp = createMockMsalApp();
      mockMsalApp.getAuthCodeUrl.mockRejectedValue(
