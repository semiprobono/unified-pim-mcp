...
import { GraphApiMockStrategy, NetworkFailureSimulator } from '../mocks/advancedMockStrategies';
  ...
  describe('Network Failure Scenarios', () => {
    ...
    test('should handle intermittent network failures', async () => {
      ...
      try {
        ...
        console.log(
          `📊 Intermittent failure results: ${successful.length} successful, ${failed.length} failed`
        );
    ...
    test('should handle DNS resolution failures', async () => {
      ...
      try {
        ...
        nock('https://invalid-graph-api.microsoft.com').get('/v1.0/me/messages').replyWithError({
          code: 'ENOTFOUND',
          errno: -3008,
          syscall: 'getaddrinfo',
  ...
  describe('API Rate Limiting and Throttling', () => {
    test('should handle aggressive rate limiting', async () => {
      ...
      try {
        ...
        console.log(
          `📊 Rate limiting results: ${successful.length} successful, ${rateLimited.length} rate limited`
        );
  ...
  describe('Service Degradation Scenarios', () => {
    test('should handle partial service outage', async () => {
      ...
      try {
        ...
        nock('https://graph.microsoft.com').persist().get('/v1.0/me').reply(200, {
          id: 'test-user',
  ...
  describe('Data Corruption and Validation', () => {
    ...
    test('should handle missing required fields in API response', async () => {
      ...
      try {
        ...
        nock('https://graph.microsoft.com').get('/v1.0/me/messages').query(true).reply(200, {
