...
import {
  MsalAuthProvider,
  TokenCacheEntry,
} from '../../../../../../src/infrastructure/adapters/microsoft/auth/MsalAuthProvider';
...
describe('MsalAuthProvider', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create public client when specified', () => {
      const publicAuthProvider = new MsalAuthProvider(
        mockConfig,
  ...
  describe('getAuthorizationUrl', () => {
    it('should generate authorization URL with PKCE', async () => {
      ...
      expect(authUrl).toBe(
        'https://login.microsoftonline.com/test-tenant/oauth2/v2.0/authorize?test=true'
      );
  ...
  describe('acquireTokenByCode', () => {
    it('should exchange authorization code for tokens', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        'Successfully acquired tokens by authorization code'
      );
    });
    ...
    it('should handle invalid grant errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        'Failed to acquire token by code',
        expect.any(Object)
      );
    });
  });
  ...
  describe('refreshAccessToken', () => {
    it('should refresh access token using refresh token', async () => {
      ...
      const tokenEntry = await authProvider.refreshAccessToken(
        'test-refresh-token',
        mockDefaultScopes
      );
  ...
  describe('getToken', () => {
    ...
    it('should fall back to refresh token if silent fails', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        'Silent token acquisition failed, attempting refresh'
      );
    });
  ...
  describe('error scenarios', () => {
    it('should handle malformed authentication results', async () => {
      ...
      (mockConfidentialClientApplication as any).acquireTokenByCode = jest
        .fn<() => Promise<any>>()
        .mockResolvedValue(malformedResult);

      await authProvider.getAuthorizationUrl();

      await expect(authProvider.acquireTokenByCode('test-code')).rejects.toThrow(
  ...
  describe('edge cases', () => {
    it('should handle missing expiration date', async () => {
      ...
      (mockConfidentialClientApplication as any).acquireTokenByCode = jest
        .fn<() => Promise<any>>()
        .mockResolvedValue(resultWithoutExpiry);
