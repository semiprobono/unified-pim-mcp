...
import {
  ...
  CircuitBreakerStats,
  CircuitState,
} from '../../../../../../src/infrastructure/adapters/microsoft/clients/CircuitBreaker';
...
describe('CircuitBreaker', () => {
  ...
  describe('execute - CLOSED state', () => {
    ...
    it('should transition to OPEN after threshold failures', async () => {
      const error = new Error('Test error');
      const mockFailFn = jest.fn<() => Promise<never>>().mockRejectedValue(error);
      const mockSuccessFn = jest.fn<() => Promise<string>>().mockResolvedValue('success');

      // Make a few successful requests first to meet volume threshold
    ...
    it('should open based on error percentage threshold', async () => {
      ...
      await percentageCB.execute(successFn);
      try {
        await percentageCB.execute(errorFn);
      } catch {}
      try {
        await percentageCB.execute(errorFn);
      } catch {}
  ...
  describe('execute - HALF_OPEN state', () => {
    ...
    it('should close circuit after enough successes', async () => {
      ...
      for (let i = 1; i < config.successThreshold; i++) {
        // i=1 because one was already executed in beforeEach
        await circuitBreaker.execute(mockFn);
      }
  ...
  describe('performance and concurrency', () => {
    ...
    it('should maintain performance under load', async () => {
      ...
      // Execute many calls
      const promises = Array.from({ length: 100 }, () => circuitBreaker.execute(mockFn));
